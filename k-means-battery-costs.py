...
def main():
    total_costs = []
    for i in range(len(cfg.battery_sizes_MW)):
        num_batteries = cfg.n_batteries_arr[i]
        battery_size = cfg.battery_sizes_MW[i]
        total_costs.append((cfg.base_cost * num_batteries) + (battery_size * num_batteries * cfg.cost_per_KWh * cfg.k_battery_day_KWh))

    for iter in range(len(cfg.battery_sizes_MW)):
        zip_coords = []
        zip_weights = []
        zip_map = []
        ...
        battery_locations = []
        solved_zip_indexes = []
        remaining_battery_indexes = [i for i in range(cfg.n_batteries_arr[iter])]
        battery_energies = [cfg.battery_sizes_MW[iter] * cfg.k_battery_day_KWh * 30 for i in range(cfg.n_batteries_arr[iter])]
        battery_supplied_zipcodes = [col.defaultdict(float) for i in range(cfg.n_batteries_arr[iter])]
